...
module.exports = {
    ...
    lister: function (callback) {
        var sql =`SELECT *, DATEDIFF(mis_fin, mis_debut)+1 AS mis_jour, DATE_FORMAT(mis_debut, '%d/%m/%Y') as mis_debut, DATE_FORMAT(mis_fin, '%d/%m/%Y') as mis_fin FROM communes, salaries, agences, missions, distances WHERE com_id = age_idCom AND sal_idAgence = age_id AND mis_idSal = sal_id AND ((dis_idComA = mis_idSalCom AND dis_idComB = mis_idCom) OR (dis_idComA = mis_idCom AND dis_idComB = mis_idSalCom)) ORDER BY mis_id` 
